...
// Affiche toute erreur JS directement à l'écran (utile sans accès aux outils de développeur).
window.addEventListener('error', (e) => {
  alert('ERREUR JS : ' + e.message + '\nFichier : ' + e.filename + '\nLigne : ' + e.lineno);
});
// Capte aussi les promesses rejetées sans .catch() (ex. un appel Supabase qui échoue) —
// ce cas ne déclenche PAS l'événement 'error' ci-dessus, il lui faut son propre écouteur.
window.addEventListener('unhandledrejection', (e) => {
  alert('PROMESSE REJETÉE (async) : ' + (e.reason?.message || e.reason?.error_description || JSON.stringify(e.reason)));
});
// Si le lot n'est toujours pas affiché après 6 secondes, la requête Supabase est probablement
// bloquée/sans réponse plutôt qu'en erreur — utile pour distinguer un plantage silencieux
// d'une requête qui ne se termine jamais.
setTimeout(() => {
  if (document.getElementById('lot-title')?.textContent === 'Chargement du lot…') {
    alert('DIAGNOSTIC : toujours en chargement après 6s — probablement une requête réseau bloquée, pas un plantage JS classique.');
  }
}, 6000);
...
document.getElementById('bid-form').addEventListener('submit', async (e) => {
  ...
  const isAutoConfirm = document.getElementById('bid-mode-auto').checked;
  const confirmMsg = isAutoConfirm
    ? `Confirmer ton enchère automatique avec un plafond de ${formatEUR(amount)} ?\n\nLe système enchérira pour toi, palier par palier, jusqu'à ce montant maximum si besoin.`
    : `Confirmer ton enchère de ${formatEUR(amount)} ?\n\nUne fois validée, tu ne peux pas te rétracter.`;
